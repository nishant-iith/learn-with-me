...
For multiclass problems, we use **Categorical Cross Entropy**.
\begin{equation}
    J = - \sum_{i=1}^{n} \sum_{k=1}^{K} y_{ik} \log(\hat{y}_{ik})
\end{equation}
Where:
\begin{itemize}
    \item $y_{ik}$: 1 if sample $i$ belongs to class $k$, else 0 (One-Hot Encoded).
    \item $\hat{y}_{ik}$: Predicted probability that sample $i$ belongs to class $k$.
\end{itemize}

\begin{quote}
\textbf{Note}: This formula requires labels to be **One-Hot Encoded**. If you are unfamiliar with One-Hot terms like `[0, 0, 1, 0]`, refer to \textbf{Appendix \ref{chap:preprocessing}: Data Preprocessing}.
\end{quote}% ========================================
...
\textbf{MULTICLASS CLASSIFICATION - CHEAT SHEET}
\vspace{0.3cm}

\textbf{Softmax Formula} (K classes):
$$P(y=k|x) = \frac{e^{z_k}}{\sum_{j=1}^{K}e^{z_j}}$$

\textbf{Categorical Cross Entropy}:
$$J = -\sum_{i}\sum_{k} y_{ik} \log(\hat{y}_{ik})$$

\textbf{Strategies}:
\begin{itemize}
    \item \textbf{OvR}: K binary models (Class vs Rest)
    \item \textbf{OvO}: K(K-1)/2 models (Pairwise)
    \item \textbf{Softmax}: Single model, K outputs
\end{itemize}

\textbf{Sklearn}:
\begin{itemize}
    \item OvR: \texttt{LogisticRegression(multi\_class='ovr')}
    \item Softmax: \texttt{LogisticRegression(multi\_class='multinomial')}
\end{itemize}

\textbf{Interview Gold}:
\begin{itemize}
    \item Softmax = mutually exclusive classes only
    \item Multi-label = use K independent Sigmoids
    \item Outputs sum to 1 (probability distribution)
\end{itemize}
}}
