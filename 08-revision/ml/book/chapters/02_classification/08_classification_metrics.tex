...
% ========================================
% SECTION 7: MULTICLASS METRICS
% ========================================
\section{Multiclass Classification Metrics}
For problems with more than 2 classes, we need to extend these metrics.

\subsection{Multiclass Confusion Matrix}
For $K$ classes, the confusion matrix is a $K \times K$ table.

\begin{table}[htbp]
    \centering
    \begin{tabular}{|l|c|c|c|}
    \hline
    & \textbf{Pred: Setosa} & \textbf{Pred: Versicolor} & \textbf{Pred: Virginica} \\ \hline
    \textbf{Actual: Setosa} & 50 & 0 & 0 \\ \hline
    \textbf{Actual: Versicolor} & 0 & 45 & 5 \\ \hline
    \textbf{Actual: Virginica} & 0 & 3 & 47 \\ \hline
    \end{tabular}
    \caption{Confusion Matrix for Iris Dataset (3 Classes)}
    \label{tab:multiclass_cm}
\end{table}

\textbf{Reading}: Rows are actual classes, columns are predicted classes. Diagonal = correct predictions.

\subsection{Per-Class Precision, Recall, and F1}
For each class $c$, we treat it as a binary problem:
\begin{itemize}
    \item \textbf{TP$_c$}: Correctly predicted as class $c$.
    \item \textbf{FP$_c$}: Predicted as $c$ but was actually another class.
    \item \textbf{FN$_c$}: Actually $c$ but predicted as another class.
\end{itemize}

\textbf{Example for Versicolor} (from Table \ref{tab:multiclass_cm}):
\begin{itemize}
    \item TP = 45 (Versicolor correctly predicted)
    \item FP = 3 (Virginica mispredicted as Versicolor)
    \item FN = 5 (Versicolor mispredicted as Virginica)
    \item Precision$_{\text{Versicolor}}$ = $\frac{45}{45+3} = 0.94$
    \item Recall$_{\text{Versicolor}}$ = $\frac{45}{45+5} = 0.90$
\end{itemize}

\subsection{Averaging Strategies}
To get a single number for the whole model, we average per-class metrics.

\begin{definition}
\textbf{Macro Average}: Simple average of per-class metrics. Treats all classes equally.
\begin{equation}
...
\begin{definition}
\textbf{Weighted Average}: Average weighted by the number of samples in each class (support).
\begin{equation}
    \text{Weighted F1} = \sum_{c=1}^{K} \frac{n_c}{N} \cdot F1_c
\end{equation}
\end{definition}

\begin{definition}
\textbf{Micro Average}: Calculate TP, FP, FN globally across all classes, then compute metrics.
\begin{equation}
    \text{Micro Precision} = \frac{\sum TP_c}{\sum TP_c + \sum FP_c}
\end{equation}
\end{definition}
...
\begin{tabular}{|l|l|}
\hline
\textbf{Averaging} & \textbf{Use When...} \\ \hline
Macro & All classes are equally important \\ \hline
Weighted & You want to account for class imbalance \\ \hline
Micro & You care about overall accuracy \\ \hline
\end{tabular}
\end{center}
