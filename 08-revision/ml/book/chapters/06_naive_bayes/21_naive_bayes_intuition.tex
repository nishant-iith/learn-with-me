...
\textbf{NAIVE BAYES - CHEAT SHEET}
\vspace{0.3cm}

\textbf{Core Formula}:
$$P(y|X) \propto P(y) \cdot \prod_{i=1}^{n} P(x_i|y)$$

\textbf{Key Assumption}: Features are conditionally independent given class

\textbf{Types}:
\begin{itemize}
    \item \textbf{Bernoulli}: Binary features (word present/absent)
    \item \textbf{Multinomial}: Count features (word frequency)
    \item \textbf{Gaussian}: Continuous features (normal distribution)
\end{itemize}

\textbf{Sklearn}:
\begin{itemize}
    \item \texttt{GaussianNB()}
    \item \texttt{MultinomialNB()}
    \item \texttt{BernoulliNB()}
\end{itemize}

\textbf{Interview Gold}:
\begin{itemize}
    \item ``Naive'' = conditional independence assumption
    \item Works well despite wrong assumption (especially for text)
    \item Handles missing values naturally (just omit term)
    \item Use Laplace smoothing to avoid zero probabilities
\end{itemize}
}}
