...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../02-supervised/assets/bias_variance_tradeoff.png}
\caption{The Bias-Variance Tradeoff. We want the sweet spot in the middle.}
...
% Bias (Increases with Lambda - Simpler model)
\addplot[domain=0:10, blue, dashed] {0.5*x};
\addlegendentry{Bias}

% Variance (Decreases with Lambda - More constraints)
\addplot[domain=0:10, red, dashed] {8*exp(-0.5*x)};
\addlegendentry{Variance}

% Total Error (U-shaped)
\addplot[domain=0:10, black, thick] {0.5*x + 8*exp(-0.5*x)};
\addlegendentry{Total Error}
\end{axis}
\end{tikzpicture}
\caption{Finding the Optimal $\lambda$. Low $\lambda$ = High Variance (Overfit). High $\lambda$ = High Bias (Underfit).}
...
\textbf{REGULARIZATION - CHEAT SHEET}
\vspace{0.3cm}

\textbf{Loss Functions}:
\begin{itemize}
    \item \textbf{Ridge} (L2): $J = MSE + \lambda \sum \beta_j^2$ (Circle constraint)
    \item \textbf{Lasso} (L1): $J = MSE + \lambda \sum |\beta_j|$ (Diamond constraint)
    \item \textbf{Elastic Net}: $J = MSE + \lambda_1 \sum|\beta| + \lambda_2 \sum\beta^2$
\end{itemize}
...
\begin{tabular}{|l|l|}
\hline
\textbf{Scenario} & \textbf{Use} \\ \hline
Many features, all important & Ridge \\ \hline
Feature selection needed & Lasso \\ \hline
Correlated features + selection & Elastic Net \\ \hline
\end{tabular}
\end{center}

\textbf{CRITICAL}: Always scale features before regularization!

\textbf{Sklearn}:
\begin{itemize}
    \item \texttt{Ridge(alpha=1.0)}
    \item \texttt{Lasso(alpha=1.0)}
    \item \texttt{ElasticNet(alpha=1.0, l1\_ratio=0.5)}
\end{itemize}

\textbf{Interview Gold}:
\begin{itemize}
    \item Lasso = feature selection (zeros coefficients)
    \item Ridge = shrinks all, removes none
    \item High $\lambda$ = more bias, less variance
\end{itemize}
}}
