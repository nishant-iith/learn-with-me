...
% ========================================
% SECTION 3: THE DUAL PROBLEM
% ========================================
\section{The Dual Problem (Why Kernels Work)}
To solve the Primal Problem, we use \textbf{Lagrange Multipliers}. This converts the minimization problem into a maximization problem (The Dual Form).

\subsection{The Lagrangian}
$$ L(w, b, \alpha) = \frac{1}{2}||w||^2 - \sum_{i=1}^{n} \alpha_i (y_i(w^T x_i + b) - 1) $$
Where $\alpha_i \ge 0$ are the Lagrange Multipliers.

Taking derivatives and setting to zero, we get the \textbf{Dual Objective}:
\begin{equation}
    \max_{\alpha} \sum_{i=1}^{n} \alpha_i - \frac{1}{2} \sum_{i=1}^{n} \sum_{j=1}^{n} \alpha_i \alpha_j y_i y_j (x_i^T x_j)
\end{equation}
Subject to $\sum \alpha_i y_i = 0$ and $\alpha_i \ge 0$.

\subsection{The Magic of Dot Products}
Notice the term $(x_i^T x_j)$. The optimization depends \textit{only} on the \textbf{Dot Product} of data points, not their absolute coordinates.
\\ This is the key that enables the **Kernel Trick**. We can replace $(x_i^T x_j)$ with a Kernel function $K(x_i, x_j)$ that computes the dot product in a higher dimension without actually going there.

\subsection{Why switch to Dual?}
\begin{enumerate}
    \item \textbf{Computational Efficiency}: The Primal Form scales with number of features ($D$). The Dual Form scales with number of samples ($N$). If $D \gg N$ (or $D$ is infinite, as in RBF Kernel), Primal is impossible, but Dual is easy.
    \item \textbf{Kernel Trick}: We cannot use Kernels in the Primal form because we need explicit coordinates $w$. In Dual form, we only need dot products.
\end{enumerate}
