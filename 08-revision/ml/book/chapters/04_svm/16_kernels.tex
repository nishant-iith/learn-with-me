...
\textbf{SVM KERNELS - CHEAT SHEET}
\vspace{0.3cm}

\textbf{Kernel Trick}: Compute dot products in high-D without explicit transformation

\textbf{Common Kernels}:
\begin{itemize}
    \item \textbf{Linear}: $K(x,y) = x^Ty$ (No transformation)
    \item \textbf{Polynomial}: $K(x,y) = (γx^Ty + r)^d$
    \item \textbf{RBF/Gaussian}: $K(x,y) = e^{-γ||x-y||^2}$ (Infinite D!)
\end{itemize}

\textbf{Hyperparameters}:
\begin{itemize}
    \item $C$: Penalty for misclassification (High C = Hard margin)
    \item $γ$ (gamma): RBF radius (High γ = Tight fit, overfitting risk)
\end{itemize}

\textbf{Sklearn}: \texttt{SVC(kernel='rbf', C=1.0, gamma='scale')}

\textbf{Interview Gold}:
\begin{itemize}
    \item Kernel Trick = magic for non-linear separation
    \item RBF maps to infinite dimensions (via Taylor expansion)
    \item High gamma = complex boundary = overfit
\end{itemize}
}}
